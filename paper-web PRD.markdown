...
感谢Chatbox在API弹出框方面的设计参考，感谢项目参考。
...
- --只要点击了运行处理，查看结果和下载表格都变成灰色--
- 只要点击运行处理，只有logo和进度条显示，其他都关闭
- -- moonshot时，关闭字段的对勾选项 --
- API那块也有显示重叠的问题
- 前端对于运行时间的计算也有问题
- 前端对于表格的显示问题（颜色，每格子宽度）
...
1. 错误可以自动分析，这样就不用每次选中然后处理了
2. CodeRider界面优化，尤其是代码片段中，比如代码类型条变窄，加入直接插入按钮
3. 加入网页搜索，比如用户哪个功能不清楚时，可以搜索官方网页
4. 在逻辑上面的问题也是很难的：比如1s后如果主函数仍在执行就运行该函数
5. 插入部分是刚需
